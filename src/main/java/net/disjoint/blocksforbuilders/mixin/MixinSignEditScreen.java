...
public class MixinSignEditScreen {
    ...
    private SpriteIdentifier getSignTextureId(SpriteIdentifier spriteIdentifier) {
        if (sign.getCachedState().getBlock() instanceof WoodSign) {
            return new SpriteIdentifier(TexturedRenderLayers.SIGNS_ATLAS_TEXTURE, ((WoodSign) sign.getCachedState().getBlock()).getTexture());
        }
